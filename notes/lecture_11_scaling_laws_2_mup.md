...
“Gradient noise”是不同小批样本给出的 gradient 不完全相同。若每个样本 gradient \(g_i\) 独立、同分布，且单样本方差是 \(\operatorname{Var}(g)\)，batch 平均
...
=\frac1{B^2}\sum_{i=1}^B\operatorname{Var}(g_i)
=\frac{B\operatorname{Var}(g)}{B^2}
=\frac{\operatorname{Var}(g)}B.
\]
...
它的 \(U=V=I\)，\(\Sigma=\operatorname{diag}(3,1)\)。理想正交化后：
...
对对角矩阵 \(\operatorname{diag}(3,1)\)，最大放大倍数是 3，所以谱范数是 3。
...
\[
\operatorname{onehot}[1,V]\ @\ W^E[V,M]
...
\[
h[1,M]\ @\ W^U[M,V]
\longrightarrow \operatorname{logits}[1,V].
\]

例：\(V=3,M=2\)，token ID=1：

\[
\operatorname{onehot}=[0,1,0],\qquad
W^E=
\begin{bmatrix}
1&2\\
3&4\\
...
\[
h_{\text{col}}[M,1]=(W^E)^\top[M,V]\operatorname{onehot}_{\text{col}}[V,1],
\]

\[
\operatorname{logits}_{\text{col}}[V,1]=(W^U)^\top[V,M]h_{\text{col}}[M,1].
\]
...
30. **【手算+解释】**\(B=\operatorname{diag}(3,1)\)。写 SVD 的 \(U,\Sigma,V\) 和理想 \(UV^\top\)。两个方向的更新比例怎样变？
31. **【画 shape】**\(h_{l-1}\) 宽 3，\(W_l\) 输出宽 2。写 \(h_{l-1},W_l,h_l\) shapes；用 §13 的矩阵算输出。
32. **【手算】**算向量 \([3,4]\) 的 norm；\(\operatorname{diag}(3,1)\) 的 spectral norm。
...
3. \(2\)B \(=2\times10^9\)，\(30\)B \(=30\times10^9\)：
   \[
   C=6(2\times10^9)(30\times10^9)
   =360\times10^{18}
   =3.6\times10^{20}\text{ FLOPs}.
   \]
...
7. A：
   \[
   d_{ff}/d_m=10/4=2.5,\quad
   d_m/heads=4/2=2,\quad
   L/d_m=2/4=0.5.
   \]
   B：
   \[
...
17. 
    \[
    16^{-0.125}=16^{-1/8}=(2^4)^{-1/8}
    =2^{-1/2}=1/\sqrt2\approx0.7071.
    \]
...
20. 
    \[
    C=(4.3\times10^{11})(1.04\times10^{12})
    =(4.3\times1.04)\times10^{23}
    =4.472\times10^{23}\text{ FLOPs}.
    \]
...
25. 
    \[
    4^{0.307}=e^{0.307\ln4}
    \approx e^{0.4256}\approx1.530.
    \]
    \[
    4^{0.571}=e^{0.571\ln4}
    \approx e^{0.7916}\approx2.207.
    \]
...
32. 
    ...
    \]
    \(\operatorname{diag}(3,1)\) 最大放大倍数是 3，所以 spectral norm 3。
...
46. 
    \[
    \operatorname{RMS}(x)=\sqrt{(3^2+4^2)/2}
    =\sqrt{25/2}=\sqrt{12.5}\approx3.5355.
    \]
    输出：
    \[
    [3/3.5355,4/3.5355]
...
60. 行向量代码约定：
    \[
    \operatorname{onehot}[1,V]@W^E[V,M]\to h[1,M],
    \]
    hidden matrix 例如 \(h[1,M]@W[M,F]\to[1,F]\)，输出：
    \[
    h[1,M]@W^U[M,V]\to\operatorname{logits}[1,V].
    \]
    列向量约定把矩阵转置：
    \[
    (W^E)^\top[M,V]\operatorname{onehot}_{col}[V,1]\to h_{col}[M,1],
    \]
    \[
