...
$$
y=x+\operatorname{Norm}_{out}\left(F(\operatorname{Norm}_{in}(x))\right).
$$
...
$$
u=x+\operatorname{Attention}(N_1(x)),
$$

$$
y=u+\operatorname{FFN}(N_2(u)).
$$
...
$$
\operatorname{RMS}(x)=\sqrt5\approx2.236.
$$
...
$$
\operatorname{ReLU}(g)=[0,2].
$$

逐元素相乘：

$$
u\odot\operatorname{ReLU}(g)
=[2\times0,(-1)\times2]=[0,-2].
$$
...
$$
\operatorname{softmax}\left(\frac{QK^{\mathsf T}}{\sqrt{d_h}}\right).
$$

为了让位置关系进入这组点积，标准 RoPE 对 Q、K 旋转：

$$
Q'=\operatorname{RoPE}(Q,pos),\qquad
K'=\operatorname{RoPE}(K,pos).
$$
...
$$
\hat q_i=\operatorname{Norm}(q_i),
$$

$$
\hat k_j=\operatorname{Norm}(k_j),
$$
...
18. $\operatorname{ReLU}(g)=[0,2]$；逐元素乘：
...
20. 等预算：
    ...
    $$
    3d d_g=2d d_{ff,plain}
    \Rightarrow d_g=\frac23d_{ff,plain}.
    $$
...
24. 先写课件与本文约定：$\delta=i-j$，所以 $j-i=-\delta$。两条差角公式是：
    ...
    =-\left(\sin j\cos i-\cos j\sin i\right)
    =-\sin(j-i),
    $$
...
28. 
    ...
    $$
    Hd_h/d=128\times128/1{,}024=16{,}384/1{,}024=16.
    $$
